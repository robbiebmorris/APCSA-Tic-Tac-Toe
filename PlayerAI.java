...
public class PlayerAI extends Board {
  ...
  public static int[] normalAI(Board board, Node letter) {// 4.make a random number generator that finds the values that are not taken
    // (board and node.ajva)
    ...
    for (int row = 0; row < boardSize; row++) {
      for (int col = 0; col < boardSize; col++) {
        if (!board.isCellFull(row, col)) {
          board.placeNode(row, col, letter);// this validifies whether the character is the winning character and places it in the grid
    ...
    if( board.getNode (1, 1) == Node.EMPTY) {
      move[0] = 1;
      move[1] = 1;
      return move;
    }
